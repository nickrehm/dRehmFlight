...
/*========================================================================================================================//
2023-10-22 Ported from Teensy to ESP32
...
Chang m1_command_PWM, server1_command_PWM, ... from int to float for increased PWM resolution
...
The heaviest function is getIMUdata(), which takes 250 us, so the rest only takes 150 us.
...
#define USE_PPM_RX
//#define USE_PWM_RX //TODO - not ported to ESP32
//#define USE_SBUS_RX //TODO - not ported to ESP32
//#define USE_DSM_RX //TODO - not ported to ESP32
...
#include "src/ESP32_PWM/ESP32_PWM.h"
...
//IMU:
#if defined USE_MPU6050_I2C
  const int wireSdaPin = 19; //ESP32 default 21
  const int wireSclPin = 18; //ESP32 default 22
  MPU6050 mpu6050;  
#elif defined USE_MPU9250_SPI
  const int spiMosiPin = 23; //ESP32 default VSPI 23 or HSPI 13
  const int spiMisoPin = 19; //ESP32 default VSPI 19 or HSPI 12
  const int spiClkPin = 18; //ESP32 default VSPI 18 or HSPI 14
  const int spiCsPin = 5; //ESP32 default VSPI 5 or HSPI 15
  SPIClass *spi = new SPIClass(VSPI);
  MPU9250 mpu9250(*spi, spiCsPin); 
#endif

//Radio:
#if defined USE_PPM_RX
  const int PPM_Pin = 35;
#elif defined USE_PWM_RX
  #error USE_PWM_RX not ported to ESP32
#elif defined USE_SBUS_RX
  #error USE_SBUS_RX not ported to ESP32
#elif USE_DSM_RX
  #error USE_DSM_RX not ported to ESP32  
#endif

//NOTE: for ESP32 it is recommended to use only pins 2,4,12-19,21-23,25-27,32-33 for motors/servos

//Motors:
//Uncomment only one set of MOTOR_XXX defines, Oneshot or PWM
//Timings for Oneshot ESCs
//#define MOTOR_FRQ 2000
//#define MOTOR_OFF 120
//#define MOTOR_MIN 125
//#define MOTOR_MAX 250
//Timings for PWM ESCs
#define MOTOR_FRQ 400
#define MOTOR_OFF 900
#define MOTOR_MIN 1000
#define MOTOR_MAX 2000
//OneShot125/PWM ESC pin output objects (pin, frequency (Hz), minimum (us), maximum (us))
PWM motor1(12,MOTOR_FRQ,MOTOR_OFF,MOTOR_MAX);
PWM motor2(13,MOTOR_FRQ,MOTOR_OFF,MOTOR_MAX);
PWM motor3(14,MOTOR_FRQ,MOTOR_OFF,MOTOR_MAX);
PWM motor4(15,MOTOR_FRQ,MOTOR_OFF,MOTOR_MAX);
PWM motor5(16,MOTOR_FRQ,MOTOR_OFF,MOTOR_MAX);
PWM motor6(17,MOTOR_FRQ,MOTOR_OFF,MOTOR_MAX);

//Servos:
#define SERVO_FRQ 50
#define SERVO_MIN 900
#define SERVO_MAX 2100
//Create servo objects to control a servo or ESC with PWM (pin, frequency (Hz), minimum (us), maximum (us))
PWM servo1(21,SERVO_FRQ,SERVO_MIN,SERVO_MAX);
PWM servo2(22,SERVO_FRQ,SERVO_MIN,SERVO_MAX);
PWM servo3(25,SERVO_FRQ,SERVO_MIN,SERVO_MAX);
PWM servo4(26,SERVO_FRQ,SERVO_MIN,SERVO_MAX);
PWM servo5(27,SERVO_FRQ,SERVO_MIN,SERVO_MAX);
PWM servo6(32,SERVO_FRQ,SERVO_MIN,SERVO_MAX);
PWM servo7(33,SERVO_FRQ,SERVO_MIN,SERVO_MAX);
...
//General stuff
float dt;
uint32_t loop_us; //runtime of loop
...
float m1_command_PWM, m2_command_PWM, m3_command_PWM, m4_command_PWM, m5_command_PWM, m6_command_PWM;
float s1_command_scaled, s2_command_scaled, s3_command_scaled, s4_command_scaled, s5_command_scaled, s6_command_scaled, s7_command_scaled;
float s1_command_PWM, s2_command_PWM, s3_command_PWM, s4_command_PWM, s5_command_PWM, s6_command_PWM, s7_command_PWM;
...
void setup() {
  Serial.begin(115200); 
  delay(500);
  Serial.println("\ndRehmFlight_ESP32 Startup");
  
  //Initialize all pins
  pinMode(ledPin, OUTPUT); //Pin, LED blinker on board

  //Motor and Servo pins initialized in constructor
  ...
  //Arm servo channels
  servo1.writeMicroseconds(SERVO_MIN); //Command servo angle from 0-180 degrees (1000 to 2000 PWM)
  servo2.writeMicroseconds(SERVO_MIN); //Set these to 1500 for servos if you do not want them to briefly max out on startup
  servo3.writeMicroseconds(SERVO_MIN); //Keep these at 1000 if you are using servo outputs for motors
  servo4.writeMicroseconds(SERVO_MIN);
  servo5.writeMicroseconds(SERVO_MIN);
  servo6.writeMicroseconds(SERVO_MIN);
  servo7.writeMicroseconds(SERVO_MIN);
  ...
  m1_command_PWM = MOTOR_MIN; //Command OneShot125 ESC from 125 to 250us pulse length
  m2_command_PWM = MOTOR_MIN;
  m3_command_PWM = MOTOR_MIN;
  m4_command_PWM = MOTOR_MIN;
  m5_command_PWM = MOTOR_MIN;
  m6_command_PWM = MOTOR_MIN;
...
void loop() {
  ...
  //printMotorCommands(); //Prints the values being written to the motors (expected: 120 to 250 for OneShot, 1000 to 2000 for PWM)
  //printServoCommands(); //Prints the values being written to the servos (expected: 1000 to 2000)
  ...
  //Write Oneshot or PWM value to servo object
  servo1.writeMicroseconds(s1_command_PWM); 
  servo2.writeMicroseconds(s2_command_PWM); 
  servo3.writeMicroseconds(s3_command_PWM);
  servo4.writeMicroseconds(s4_command_PWM);
  servo5.writeMicroseconds(s5_command_PWM);
  servo6.writeMicroseconds(s6_command_PWM);
  servo7.writeMicroseconds(s7_command_PWM);
...
void IMUinit() {
   ...
   */
  #if defined USE_MPU6050_I2C
    Wire.begin(wireSdaPin, wireSclPin, 1000000);//Note this is 2.5 times the spec sheet 400 kHz max...
  ...
  #elif defined USE_MPU9250_SPI
    spi->begin(spiMosiPin ,spiMisoPin, spiClkPin, spiCsPin);
...
void scaleCommands() {
   ...
   */  
  //Scaled to 125us - 250us for oneshot125 protocol or to servo bounds for PWM
  m1_command_PWM = m1_command_scaled*(MOTOR_MAX-MOTOR_MIN) + MOTOR_MIN;
  m2_command_PWM = m2_command_scaled*(MOTOR_MAX-MOTOR_MIN) + MOTOR_MIN;
  m3_command_PWM = m3_command_scaled*(MOTOR_MAX-MOTOR_MIN) + MOTOR_MIN;
  m4_command_PWM = m4_command_scaled*(MOTOR_MAX-MOTOR_MIN) + MOTOR_MIN;
  m5_command_PWM = m5_command_scaled*(MOTOR_MAX-MOTOR_MIN) + MOTOR_MIN;
  m6_command_PWM = m6_command_scaled*(MOTOR_MAX-MOTOR_MIN) + MOTOR_MIN;
  //Constrain commands to motors within oneshot125 bounds or to servo bounds
  m1_command_PWM = constrain(m1_command_PWM, MOTOR_MIN, MOTOR_MAX);
  m2_command_PWM = constrain(m2_command_PWM, MOTOR_MIN, MOTOR_MAX);
  m3_command_PWM = constrain(m3_command_PWM, MOTOR_MIN, MOTOR_MAX);
  m4_command_PWM = constrain(m4_command_PWM, MOTOR_MIN, MOTOR_MAX);
  m5_command_PWM = constrain(m5_command_PWM, MOTOR_MIN, MOTOR_MAX);
  m6_command_PWM = constrain(m6_command_PWM, MOTOR_MIN, MOTOR_MAX);

  //Scaled to 0-180 for servo library
  s1_command_PWM = s1_command_scaled*(SERVO_MAX-SERVO_MIN) + SERVO_MIN;
  s2_command_PWM = s2_command_scaled*(SERVO_MAX-SERVO_MIN) + SERVO_MIN;
  s3_command_PWM = s3_command_scaled*(SERVO_MAX-SERVO_MIN) + SERVO_MIN;
  s4_command_PWM = s4_command_scaled*(SERVO_MAX-SERVO_MIN) + SERVO_MIN;
  s5_command_PWM = s5_command_scaled*(SERVO_MAX-SERVO_MIN) + SERVO_MIN;
  s6_command_PWM = s6_command_scaled*(SERVO_MAX-SERVO_MIN) + SERVO_MIN;
  s7_command_PWM = s7_command_scaled*(SERVO_MAX-SERVO_MIN) + SERVO_MIN;
  //Constrain commands to servos within servo library bounds
  s1_command_PWM = constrain(s1_command_PWM, SERVO_MIN, SERVO_MAX);
  s2_command_PWM = constrain(s2_command_PWM, SERVO_MIN, SERVO_MAX);
  s3_command_PWM = constrain(s3_command_PWM, SERVO_MIN, SERVO_MAX);
  s4_command_PWM = constrain(s4_command_PWM, SERVO_MIN, SERVO_MAX);
  s5_command_PWM = constrain(s5_command_PWM, SERVO_MIN, SERVO_MAX);
  s6_command_PWM = constrain(s6_command_PWM, SERVO_MIN, SERVO_MAX);
  s7_command_PWM = constrain(s7_command_PWM, SERVO_MIN, SERVO_MAX);
}
...
void commandMotors() {
  //DESCRIPTION: Send pulses to motor pins, oneshot125 or PWM protocol
  motor1.writeMicroseconds(m1_command_PWM);
  motor2.writeMicroseconds(m2_command_PWM);
  motor3.writeMicroseconds(m3_command_PWM);
  motor4.writeMicroseconds(m4_command_PWM);
  motor5.writeMicroseconds(m5_command_PWM);
  motor6.writeMicroseconds(m6_command_PWM);    
}
...
void calibrateESCs() {
   ...
   */
   while (true) {
      ...
      scaleCommands(); //Scales motor commands to 125 to 250 range (oneshot125 protocol) and servo PWM commands to 1000 to 2000 (for servo library)
...
void throttleCut() {
  ...
  /*
      ...
      channel_5_pwm is HIGH then throttle cut is ON and throttle value = MOTOR_OFF only. (ThrottleCut is ACTIVATED), (drone is DISARMED)
  */
  if ((channel_5_pwm > 1500) || (armedFly == false)) {
    armedFly = false;
    m1_command_PWM = MOTOR_OFF;
    m2_command_PWM = MOTOR_OFF;
    m3_command_PWM = MOTOR_OFF;
    m4_command_PWM = MOTOR_OFF;
    m5_command_PWM = MOTOR_OFF;
    m6_command_PWM = MOTOR_OFF;

    //Uncomment if using servo PWM variables to control motor ESCs
    //s1_command_PWM = SERVO_MIN;
    //s2_command_PWM = SERVO_MIN;
    //s3_command_PWM = SERVO_MIN;
    //s4_command_PWM = SERVO_MIN;
    //s5_command_PWM = SERVO_MIN;
    //s6_command_PWM = SERVO_MIN;
    //s7_command_PWM = SERVO_MIN;
  }
}
...
void loopBlink() {
  ...
  if (current_time - blink_counter > blink_delay) {
    ...
    digitalWrite(ledPin, blinkAlternate);
